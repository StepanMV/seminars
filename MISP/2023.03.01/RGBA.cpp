...
export class RGB
{
    ...
    public:
        ...
        virtual void print(const PrintType& type = E_RGBA) const
        {
            switch(type)
            {
                case E_RGBA:
                ...
                case E_INT:
                    {
                        auto result = std::bitset<32>((unsigned int) ((red << 16) | (green << 8) | blue)).to_string();
        ...
        virtual RGB sum(const RGB& rgb) const
        {
            int r = this->red + rgb.getRed() > 255 ? 255 : this->red + rgb.getRed();
            int g = this->green + rgb.getGreen() > 255 ? 255 : this->green + rgb.getGreen();
            int b = this->blue + rgb.getBlue() > 255 ? 255 : this->blue + rgb.getBlue();
            return RGB(r, g, b);
        }
};

export class RGBA: public RGB
{
    protected:
        int alpha;
    ...
    public:
        ...
        RGBA(int red, int green, int blue, int alpha = 255) : RGB(red, green, blue)
        {
            this->alpha = alpha > 255 ? 255 : alpha;
        }

        RGBA(const RGBA& rgba) : RGB(rgba)
        {
        ...
        void print(const PrintType& type = E_RGBA) const override
        {
            switch(type)
            {
                case E_RGBA:
